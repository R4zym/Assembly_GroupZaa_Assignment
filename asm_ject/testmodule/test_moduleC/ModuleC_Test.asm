EXTRN GenerateKeySchedule@8 : PROC
...
; Test Vector จาก NIST (FIPS 46-3 Test Vector)
testPlaintext   BYTE 012h, 034h, 056h, 078h, 09Ah, 0BCh, 0DEh, 0EFh
testKey         BYTE 013h, 034h, 057h, 079h, 09Bh, 0BCh, 0DFh, 0F1h
expectedCipher  BYTE 085h, 0E8h, 013h, 054h, 00Fh, 00Ah, 0B4h, 005h

; Buffer สำหรับการทดสอบ
cipherBuffer    BYTE 8 DUP(0)
decryptedBuffer BYTE 8 DUP(0)

; Subkeys 16 รอบ (รอบละ 48 bits = 6 bytes, รวม 96 bytes)
SubKeys         BYTE 1Bh, 02h, EFh, FCh, 70h, 72h
                BYTE 79h, AEh, D9h, DBh, C9h, E5h
                BYTE 55h, FCh, 8Ah, 42h, CFh, 99h
                BYTE 72h, ADh, D6h, DBh, 35h, 1Dh
                BYTE 7Ch, ECh, 07h, EBh, 53h, A8h
                BYTE 63h, A5h, 3Eh, 50h, 7Bh, 2Fh
                BYTE ECh, 84h, B7h, F6h, 18h, BCh
                BYTE F7h, 8Ah, 3Ah, C1h, 3Bh, FBh
                BYTE E0h, DBh, EBh, EDh, E7h, 81h
                BYTE B1h, F3h, 47h, BAh, 46h, 4Fh
                BYTE 21h, 5Fh, D3h, DEh, D3h, 86h
                BYTE 75h, 71h, F5h, 94h, 67h, E9h
                BYTE 97h, C5h, D1h, FAh, BAh, 41h
                BYTE 5Fh, 43h, B7h, F2h, E7h, 3Ah
                BYTE BFh, 91h, 8Dh, 3Dh, 3Fh, 0Ah
                BYTE CBh, 3Dh, 8Bh, 0Eh, 17h, F5h


; =========================================================
; DES PERMUTATION TABLES (1-indexed ตามมาตรฐาน FIPS 46-3)
; =========================================================
...
main PROC
    ...
    ; 2. Use known-good NIST subkeys for isolated Module C test

    ; =========================================================
; 2. Generate Subkeys using Module B
; =========================================================
push OFFSET SubKeys
push OFFSET testKey
call GenerateKeySchedule@8
...
ColDone:
    ...
    ; เขียน 4 บิตลงใน sboxOut Buffer
    push ecx                    ; สำรอง ECX เพื่อป้องกันไม่ให้ถูกลูปย่อยทำลาย
    mov  edx, 0
...
Write4BitsLoop:
    ...
    mov  esi, [esp]             ; ดึงค่า ecx (S-Box Index) จาก stack
    imul esi, 4
    add  esi, edx
    inc  esi
...
Write4BitsDone:
    pop  ecx                    ; คืนค่า ECX เพื่อวน SBoxLoop ต่อไป
    inc  ecx
    jmp  SBoxLoop
